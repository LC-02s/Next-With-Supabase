...
import { useFollowerListInfiniteQuery, useResetFollowingQuery } from '@/entities/following'
...
const ErrorFallback: React.FC = () => {
  ...
  return (
    <div className="mx-4 flex min-h-60 flex-col items-center justify-center rounded-xl bg-gray-50 p-3 dark:bg-dark-600">
      ...
      <p className="mb-8 break-keep font-semibold">팔로워 목록을 불러오는데 실패했어요</p>
      <Button
        variant="default"
        color="gray"
        radius="md"
...
export const FollowerList: React.FC = () => {
  ...
  const {
    ...
    isFetchingNextPage,
    fetchNextPage,
  } = useFollowerListInfiniteQuery({ userId: session?.id })
  ...
  if (data.length <= 0) {
    return (
      <div className="mx-4 flex min-h-60 flex-col items-center justify-center rounded-xl bg-gray-50 p-3 dark:bg-dark-600">
        ...
        <p className="mb-8 break-keep font-semibold">팔로워가 없어요</p>
      </div>
    )
  }
